...
public class AI {
	...
	int miniMax_move(Box[][] grid, int diff, Box color) {
		...
		// move else return the bestValue
		if (diff == 0)
			return bestMove;
		else
			return bestValue;
	...
	int check_value(Box[][] grid, Box color) {
		...
		for (int row = 0; row < 6; row++) {
			for (int col = 0; col < 4; col++) {
				...
				for (int i = 0; i != 6; i++) {
					...
					// check if one of the horizontal 2 in a row possibility's
					// is true
				...
				for (int i = 0; i != 4; i++) {
					...
					// check if one of the horizontal 3 in a row possibility's
					// is true
		...
		for (int row = 0; row < 2; row++)
			for (int col = 0; col < 4; col++) {
				for (int i = 0; i != 6; i++) {
					...
					// check if one of the diagonal 2 in a row possibility's is
					// true
				...
				for (int i = 0; i != 4; i++) {
					...
					// check if one of the diagonal 3 in a row possibility's is
					// true
					if (grid[col + i1][row + i1] == color && grid[col + i2][row + i2] == color
		...
		for (int row = 0; row < 6; row++)
			for (int col = 0; col < 3; col++)
				// check if one of the horizontal open 3 in a row possibility's
				// is true
		...
		for (int row = 0; row < 2; row++) {
			if (grid[0][row + 3] == Board.empty && grid[1][row + 2] == color && grid[2][row + 1] == color
					&& grid[3][row] == color && grid[4][row + 1] == color && grid[5][row + 2] == color
					&& grid[6][row + 3] == Board.empty) {
				value += 10000;
			}
			if (grid[0][row] == Board.empty && grid[1][row + 1] == color && grid[2][row + 2] == color
					&& grid[3][row + 3] == color && grid[4][row + 2] == color && grid[5][row + 1] == color
					&& grid[6][row] == Board.empty) {
				value += 10000;
			}
		}
